"""rdholmes31_duckdb_retail.py - Project script (example).
...
def main() -> None:
    ...
    try:
        ...
        # ----------------------------------------------------
        run_sql_script(con, SQL_DIR / "rdholmes31_retail_clean.sql")

        # ----------------------------------------------------
        # STEP 2: BOOTSTRAP (create tables, load CSV data)
        # ----------------------------------------------------
        run_sql_script(con, SQL_DIR / "rdholmes31_retail_bootstrap.sql")

        # ----------------------------------------------------
        # STEP 3: RUN BASIC QUERIES
        # ----------------------------------------------------
        run_sql_query(con, SQL_DIR / "rdholmes31_retail_query_store_count.sql")
        run_sql_query(con, SQL_DIR / "rdholmes31_retail_query_sales_count.sql")
        run_sql_query(con, SQL_DIR / "rdholmes31_retail_query_sales_aggregate.sql")
        run_sql_query(con, SQL_DIR / "rdholmes31_retail_query_sales_by_category.sql")

        # ----------------------------------------------------
        # STEP 4: RUN KPI QUERY (ACTION-DRIVEN)
        # ----------------------------------------------------
        run_sql_query(con, SQL_DIR / "rdholmes31_retail_query_kpi_revenue.sql")
